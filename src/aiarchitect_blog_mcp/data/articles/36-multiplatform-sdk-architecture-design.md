...
## 작성 배경: 고객 맞춤형 채널과 프라이빗 API 사이에 안전한 경계를 만든다
...
## 1. 멀티플랫폼 SDK의 핵심 문제는 코드 중복이 아니라 의미의 분기다
...
## 2. 전체 구조: 신뢰 경계를 지키면서 계약 코어와 플랫폼 어댑터를 분리한다
...
## 3. 진실의 원천: 사람의 기억이 아니라 기계가 읽는 계약
...
## 6. 인증 계약: 토큰을 받는 방법과 보관하는 방법을 분리한다
...
## 7. 오류 계약: 메시지가 아니라 코드와 속성으로 판단한다
...
## 8. 비동기·취소 계약: 실행 결과뿐 아니라 중단 의미도 맞춘다
...
## 10. 실시간 이벤트 계약: 연결 상태와 업무 이벤트를 분리한다
...
## 11. 플랫폼별 공개 API는 관습적으로, 상태 의미는 동일하게
...
## 13. 버전 전략: 패키지 버전과 서버 API 버전을 분리한다
...
## 15. 적합성 테스트: 각 SDK가 같은 계약을 구현했음을 증명한다
...
## 16. 관측성 계약: 사용자의 로그를 오염시키지 않으면서 진단 가능하게
...
## 17. 기술문서 구조: 시작하기보다 운영하기가 더 중요하다
...
## 18. 릴리스 파이프라인: 생성·검증·패키징·문서를 한 흐름으로 묶는다
...
## 20. 마무리: 고객 채널의 자유도와 프라이빗 API 보호를 함께 달성한다
